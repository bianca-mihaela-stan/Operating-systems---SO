...
int decrease_count(int count)
{
    ...
    if(available_resources<count)//daca nu se gasesc destul resurse nu inseamna ca thread-ul cere prea multe, ci ca in momentul asta nu sunt disponibile
    {
        ...
        sleep(1);//il pun sa astepte putin
        return decrease_count(count);//si il pun sa apeleze din nou functia
    }
...
void * fct(void * arg)
{
    ...
    int res= (int) arg;
    decrease_count(res);
    //l-am am pus fiecare thread sa astepte putin, pentru ca altfel nu se suprapuneau deloc thread-urile
